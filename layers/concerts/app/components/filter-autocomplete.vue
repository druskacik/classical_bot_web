<template>
  <div ref="root" class="relative">
    ...
    >
      ...
      >
        ...
        <button
          ...
          :aria-label="t('Remove {label} filter', { label: optionChipLabel(option) })"
    ...
    >
      <p>{{ t('Options could not be loaded.') }}</p>
      <button
        type="button"
...
const statusMessage = computed(() => {
  ...
  if (loadError.value) return t('Options could not be loaded. Try again.')
  if (loading.value) return t('Searching for options.')
  const count = availableOptions.value.length
  return count ? describeOptions(count) : t('No matching options.')
})
...
const loadOptions = async () => {
  ...
  try {
    ...
    selectedOptions.value = props.modelValue.map(value => byValue.get(String(value)) || {
      value: String(value),
      label: props.type === 'work' ? t('Work {id}', { id: value }) : String(value),
    })
    activeIndex.value = -1
  } catch {
    if (sequence !== requestSequence.value) return
