...
The public, unauthenticated endpoint is `GET /api/v1/concerts`. It returns upcoming concerts already collected for an organisation website. It does not start a crawl. Deploy the application before using the production examples below; implementation alone does not publish the endpoint.
...
JSON contains `source` (`id`, `name`, `url`), `concerts`, `pagination` (`page`, `page_size`, `total`, `total_pages`, `all`, `next`) and `generated_at`. In complete exports the page-related values and next link are null. CSV contains only the concert table. Both formats expose `X-Total-Count` and, when applicable, `Link: <...>; rel="next"`. Next links are relative to the API origin and retain the selected format. Pages beyond the result set are empty.
...
Registered canonical URLs and aliases are matched first, normalizing HTTP/HTTPS, www, default ports, host case and fragments while retaining meaningful paths and query strings. An otherwise unmatched URL can resolve by hostname only if that hostname belongs to one registered source. No submitted URL is fetched. Unknown hosts return 404; ambiguous hosts return 422 with candidate canonical URLs.
...
| 422 | `ambiguous_source`, `export_too_large` |
| 429 | `rate_limited` (with Retry-After seconds) |
| 500 | `internal_error` |

A registered organisation with no upcoming concerts returns 200 and an empty collection/header-only CSV. GET and OPTIONS allow public CORS without credentials; pagination/download headers are exposed.
