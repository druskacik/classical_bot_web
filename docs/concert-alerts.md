...
1. Disable `NUXT_PUBLIC_ALERTS_SIGNUP_ENABLED` and `ALERTS_SENDING_ENABLED` and stop the old worker before applying the migration and deploying the updated web app. Do not run old application code against the new schema.
...
5. Enable signup with `NUXT_PUBLIC_ALERTS_SIGNUP_ENABLED=true`. Enable digests separately with `ALERTS_SENDING_ENABLED=true`. Keep digest sending off in development. `ALERTS_DAILY_LIMIT` defaults to 100 SMTP attempts across all subscribers. Public confirmation limits remain three/email/hour, five/IP/15 minutes, and 30 globally/hour; they do not cap saved alert counts.
