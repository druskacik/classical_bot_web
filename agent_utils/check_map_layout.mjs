...
try {
  ...
  const settlePanel = () => page.waitForFunction(() => {
    const sheet = document.querySelector('.map-sheet.is-mobile')
    return !sheet || sheet.getAnimations().every(animation => animation.playState === 'finished')
  })
  for (const [width, height] of (process.env.MAP_TILE_FAILURE_ONLY ? [] : process.env.MAP_MOTION_ONLY ? [[390, 667]] : [[1440, 900], [1440, 700], [1024, 768], [320, 667], [390, 844], [390, 667], [430, 932], [844, 390]])) {
    ...
    if (mobile) {
      ...
      if (width === 390 && height === 667) {
        ...
        const panelTop = await page.$eval('.map-panel', el => el.getBoundingClientRect().top)
        await page.mouse.move(handle.x, handle.y - 40, { steps: 4 })
        await page.waitForFunction(top => Math.abs(document.querySelector('.map-panel').getBoundingClientRect().top - (top - 40)) < 2, {}, panelTop)
        assert.ok(await page.$('.map-panel.is-explore'), 'Dragging moves the panel before committing its next state')
        assert.equal((await measure()).mapHeight, initial.mapHeight, 'Direct dragging must not resize the map')
        await page.mouse.move(handle.x, handle.y - 80, { steps: 4 })
        await page.mouse.up()
        await page.waitForSelector('.is-preview')
        assert.ok(await page.$eval('.map-sheet', el => el.getAnimations().some(animation => animation.playState === 'running')), 'Release must animate the snap using transform')
        await settlePanel()
        const previewHandle = await page.$eval('.panel-handle', el => { const r = el.getBoundingClientRect(); return { x: r.x + r.width / 2, y: r.y + 1 } })
        await page.mouse.move(previewHandle.x, previewHandle.y)
        await page.mouse.down()
        await page.mouse.move(previewHandle.x, previewHandle.y + 40, { steps: 4 })
        await page.$eval('.panel-header', el => el.dispatchEvent(new PointerEvent('pointercancel', { bubbles: true })))
        await page.mouse.up()
        await settlePanel()
        assert.ok(await page.$('.is-preview'), 'Cancelled dragging returns to the current state')
        const previewTop = await page.$eval('.map-panel', el => el.getBoundingClientRect().top)
        await page.mouse.move(previewHandle.x, previewHandle.y)
        await page.mouse.down()
        await page.mouse.move(previewHandle.x, previewHandle.y + 50, { steps: 4 })
        await page.waitForFunction(top => Math.abs(document.querySelector('.map-panel').getBoundingClientRect().top - (top + 50)) < 2, {}, previewTop)
        await page.mouse.move(previewHandle.x, previewHandle.y + 100, { steps: 4 })
        await page.mouse.up()
        await page.waitForSelector('.is-explore')
        await settlePanel()
        await page.click('.panel-toggle')
        await page.waitForSelector('.is-preview')
        await settlePanel()
        ...
        await page.click('.panel-toggle')
        await page.waitForSelector('.is-read')
        assert.equal(await page.$eval('.map-sheet', el => el.getAnimations().length), 0, 'Reduced motion skips animated snapping')
        await page.click('.panel-back')
        await page.waitForSelector('.is-explore')
