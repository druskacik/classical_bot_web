...
- City and country retain the existing deterministic colored outline badges, rendered with regular 1px borders rather than inset rings. Linked badges use a 5% tint of their text color on hover, with 150ms background feedback and no underline; keyboard focus retains its outline. Gray-600 source text accompanies them in a wrapping row with 0.5rem gaps. Metadata links retain a compact 1.75rem minimum height on mobile with a 0.25rem row gap; coarse-pointer metadata links from 640px have 2.75rem minimum targets. Composer-only links use the same 1.5rem mobile minimum height as programme links.
- From 640px, programme starts after a 1rem margin and 0.75rem top padding with a gray-200 rule. On mobile, programme uses the same 0.5rem content gap as composer-only lists, with no extra margin, padding, or rule. Composer names use gray-700, semibold below 640px and regular from 640px in both programme rows and composer-only lists; work links are gray-900. From 640px, each group has an 11rem composer column and a 1.5rem gap; below it, works stack under the composer with a 0.75rem indent. Text wraps naturally. Mobile programme links have a 1.5rem minimum height, a 0.25rem composer-to-work gap, and 0.75rem between composer groups; coarse-pointer links from 640px retain 2.75rem minimum targets.
